    ...
    if circulation_status
      ...
      if current
        content_tag :strong do
          link_to("#{circulation_status.display_name} (" + facet.count.to_s + ")", url_for(params.merge(circulation_status: circulation_status.name, page: nil, view: nil, only_path: true)))
        end
      else
        link_to("#{circulation_status.display_name} (" + facet.count.to_s + ")", url_for(params.merge(circulation_status: circulation_status.name, page: nil, view: nil, only_path: true)))
      end
